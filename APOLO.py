...
# Task 2: Compute the spacecraft's acceleration due to gravity
	...
	vector_form_earth = position
	acc = -gravitational_constant * (earth_mass / numpy.linalg.norm(vector_form_earth)
	                                   + moon_mass / numpy.linalg.norm(vector_from_moon))
...
def apply_boost():
    ...
    while current_time < total_duration:
        ...
        if not mcc2_burn_done and current_time >= 101104.:
            ...
            matplotlib.pyplot.scatter(position[0],position[1],s = 16., fac
            mcc2_burn_done = True
        ...
        velocityH = velocity + h * 0.5 * (acceleration0
                                           + acceleration(curren_time + h,)
        positionH = postion + h * 0.5 * (velocity + velocityE)
        velocity = velocityH
        position = positionH                                 
